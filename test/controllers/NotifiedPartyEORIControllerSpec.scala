...
import views.html.NotifiedPartyEORIView
...
class NotifiedPartyEORIControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val notifiedPartyEORIRoute = routes.NotifiedPartyEORIController.onPageLoad(NormalMode, lrn).url
  ...
  "NotifiedPartyEORI Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, notifiedPartyEORIRoute)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(NotifiedPartyEORIPage, "answer").success.value
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result          = route(application, request).value
        val expectedAnswers = emptyUserAnswers.set(NotifiedPartyEORIPage, "answer").success.value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual NotifiedPartyEORIPage.navigate(NormalMode, expectedAnswers).url

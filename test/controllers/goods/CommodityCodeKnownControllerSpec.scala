...
import views.html.goods.CommodityCodeKnownView
...
class CommodityCodeKnownControllerSpec extends SpecBase with MockitoSugar {
  ...
  "CommodityCodeKnown Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(goods.CommodityCodeKnownPage(index), true).success.value
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result = route(application, request).value
        val expectedAnswers =
          emptyUserAnswers.set(goods.CommodityCodeKnownPage(index), true).success.value
    ...
    "must redirect to Journey Recovery for a GET if the item index is equal to or higher than that maximum" in {
      ...
      running(application) {
        val request =
          FakeRequest(GET, routes.CommodityCodeKnownController.onPageLoad(waypoints, lrn, Index(maxGoods)).url)
    ...
    "must redirect to Journey Recovery for a POST if the item index is equal to or higher than that maximum" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, routes.CommodityCodeKnownController.onPageLoad(waypoints, lrn, Index(maxGoods)).url)

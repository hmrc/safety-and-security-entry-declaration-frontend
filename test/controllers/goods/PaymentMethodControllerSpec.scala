...
import models.{Index, PaymentMethod}
...
class PaymentMethodControllerSpec extends SpecBase with MockitoSugar {
  ...
  "PaymentMethod Controller" - {
    ...
    "must redirect to Journey Recovery for a GET if the item index is equal to or higher than that maximum" in {
      ...
      running(application) {
        val request =
          FakeRequest(GET, routes.PaymentMethodController.onPageLoad(waypoints, lrn, Index(maxGoods)).url)
    ...
    "must redirect to Journey Recovery for a POST if the item index is equal to or higher than that maximum" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, routes.PaymentMethodController.onPageLoad(waypoints, lrn, Index(maxGoods)).url)

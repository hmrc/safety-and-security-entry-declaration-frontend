...
class KindOfPackageControllerSpec extends SpecBase with MockitoSugar {
  ...
  "KindOfPackage Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, kindOfPackageRoute)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers =
        emptyUserAnswers.set(goods.KindOfPackagePage(index, index), kindOfPackage).success.value
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, kindOfPackageRoute)
    ...
    "must redirect to Journey Recovery for a GET if the item index is equal to or higher than that maximum" in {
      ...
      running(application) {
        val request =
          FakeRequest(GET, routes.KindOfPackageController.onPageLoad(waypoints, lrn, Index(maxGoods), index).url)
    ...
    "must redirect to Journey Recovery for a POST if the item index is equal to or higher than that maximum" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, routes.KindOfPackageController.onPageLoad(waypoints, lrn, Index(maxGoods), index).url)
    ...
    "must redirect to Journey Recovery for a GET if the package index is equal to or higher than that maximum" in {
      ...
      running(application) {
        val request =
          FakeRequest(GET, routes.KindOfPackageController.onPageLoad(waypoints, lrn, index, Index(maxPackages)).url)
    ...
    "must redirect to Journey Recovery for a POST if the package index is equal to or higher than that maximum" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, routes.KindOfPackageController.onPageLoad(waypoints, lrn, index, Index(maxPackages)).url)

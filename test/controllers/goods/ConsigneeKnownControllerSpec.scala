...
import views.html.goods.ConsigneeKnownView
...
class ConsigneeKnownControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val consigneeKnownRoute = routes.ConsigneeKnownController.onPageLoad(waypoints, lrn, index).url
  ...
  "ConsigneeKnown Controller" - {
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual ConsigneeKnownPage(index).navigate(waypoints, expectedAnswers).url
    ...
    "must redirect to Journey Recovery for a GET if the item index is equal to or higher than that maximum" in {
      ...
      running(application) {
        val request =
          FakeRequest(GET, routes.ConsigneeKnownController.onPageLoad(waypoints, lrn, Index(maxGoods)).url)
    ...
    "must redirect to Journey Recovery for a POST if the item index is equal to or higher than that maximum" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, routes.ConsigneeKnownController.onPageLoad(waypoints, lrn, Index(maxGoods)).url)

...
import forms.goods.GoodsDescriptionFormProvider
import models.Index
...
import views.html.goods.GoodsDescriptionView
...
class GoodsDescriptionControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val goodsDescriptionRoute =
    routes.GoodsDescriptionController.onPageLoad(waypoints, lrn, index).url
  ...
  "GoodsDescription Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(goods.GoodsDescriptionPage(index), "answer").success.value
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result = route(application, request).value
        val expectedAnswers =
          emptyUserAnswers.set(goods.GoodsDescriptionPage(index), "answer").success.value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual goods.GoodsDescriptionPage(index)
    ...
    "must redirect to Journey Recovery for a GET if the item index is equal to or higher than that maximum" in {
      ...
      running(application) {
        val request =
          FakeRequest(GET, routes.GoodsDescriptionController.onPageLoad(waypoints, lrn, Index(maxGoods)).url)
    ...
    "must redirect to Journey Recovery for a POST if the item index is equal to or higher than that maximum" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, routes.GoodsDescriptionController.onPageLoad(waypoints, lrn, Index(maxGoods)).url)

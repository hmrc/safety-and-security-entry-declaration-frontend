...
class AddMarkOrNumberControllerSpec extends SpecBase with MockitoSugar {
  ...
  "AddMarkOrNumber Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, addMarkOrNumberRoute)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(goods.AddMarkOrNumberPage(index, index), true).success.value
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, addMarkOrNumberRoute)
            .withFormUrlEncodedBody(("value", "true"))

        val result = route(application, request).value
        val expectedAnswers =
          emptyUserAnswers.set(goods.AddMarkOrNumberPage(index, index), true).success.value
    ...
    "must redirect to Journey Recovery for a GET if the item index is equal to or higher than that maximum" in {
      ...
      running(application) {
        val request =
          FakeRequest(GET, routes.AddMarkOrNumberController.onPageLoad(waypoints, lrn, Index(maxGoods), index).url)
    ...
    "must redirect to Journey Recovery for a POST if the item index is equal to or higher than that maximum" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, routes.AddMarkOrNumberController.onPageLoad(waypoints, lrn, Index(maxGoods), index).url)
    ...
    "must redirect to Journey Recovery for a GET if the package index is equal to or higher than that maximum" in {
      ...
      running(application) {
        val request =
          FakeRequest(GET, routes.AddMarkOrNumberController.onPageLoad(waypoints, lrn, index, Index(maxPackages)).url)
    ...
    "must redirect to Journey Recovery for a POST if the package index is equal to or higher than that maximum" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, routes.AddMarkOrNumberController.onPageLoad(waypoints, lrn, index, Index(maxPackages)).url)

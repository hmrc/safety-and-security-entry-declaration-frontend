...
class GoodsItemGrossWeightControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val goodsItemGrossWeightRoute =
    routes.GoodsItemGrossWeightController.onPageLoad(waypoints, lrn, index).url
  ...
  "GoodsItemGrossWeight Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, goodsItemGrossWeightRoute)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers =
        emptyUserAnswers.set(goods.GoodsItemGrossWeightPage(index), validAnswer).success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(validAnswer), waypoints, lrn, index)(
          request,
          messages(application)
        ).toString
      }
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, goodsItemGrossWeightRoute)
        ...
        redirectLocation(result).value mustEqual goods.GoodsItemGrossWeightPage(index)
    ...
    "must redirect to Journey Recovery for a GET if the item index is equal to or higher than that maximum" in {
      ...
      running(application) {
        val request =
          FakeRequest(GET, routes.GoodsItemGrossWeightController.onPageLoad(waypoints, lrn, Index(maxGoods)).url)
    ...
    "must redirect to Journey Recovery for a POST if the item index is equal to or higher than that maximum" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, routes.GoodsItemGrossWeightController.onPageLoad(waypoints, lrn, Index(maxGoods)).url)

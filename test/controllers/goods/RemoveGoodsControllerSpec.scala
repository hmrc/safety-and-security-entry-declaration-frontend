...
class RemoveGoodsControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val removeGoodsRoute =
    routes.RemoveGoodsController.onPageLoad(waypoints, lrn, index).url
  ...
  "RemoveGoods Controller" - {
    ...
    "must redirect to Journey Recovery for a GET if the item index is equal to or higher than that maximum" in {
      ...
      running(application) {
        val request =
          FakeRequest(GET, routes.RemoveGoodsController.onPageLoad(waypoints, lrn, Index(maxGoods)).url)
    ...
    "must redirect to Journey Recovery for a POST if the item index is equal to or higher than that maximum" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, routes.RemoveGoodsController.onPageLoad(waypoints, lrn, Index(maxGoods)).url)

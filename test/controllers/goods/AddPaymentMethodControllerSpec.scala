...
import forms.goods.AddPaymentMethodFormProvider
...
import views.html.goods.AddPaymentMethodView
...
class AddPaymentMethodControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val addPaymentMethodRoute =
    routes.AddPaymentMethodController.onPageLoad(NormalMode, lrn, index).url
  ...
  "AddPaymentMethod Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(goods.AddPaymentMethodPage(index), true).success.value
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result = route(application, request).value
        val expectedAnswers = emptyUserAnswers.set(goods.AddPaymentMethodPage(index), true).success.value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual goods.AddPaymentMethodPage(index)

...
class CommodityCodeControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val commodityCodeRoute = routes.CommodityCodeController.onPageLoad(NormalMode, lrn).url
  ...
  "CommodityCode Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(CommodityCodePage, "answer").success.value
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result          = route(application, request).value
        val expectedAnswers = emptyUserAnswers.set(CommodityCodePage, "answer").success.value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual CommodityCodePage.navigate(NormalMode, expectedAnswers).url

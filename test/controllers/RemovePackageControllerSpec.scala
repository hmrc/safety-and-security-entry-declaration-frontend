...
class RemovePackageControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val removePackageRoute = routes.RemovePackageController.onPageLoad(NormalMode, lrn).url
  ...
  "RemovePackage Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(RemovePackagePage, true).success.value
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result          = route(application, request).value
        val expectedAnswers = emptyUserAnswers.set(RemovePackagePage, true).success.value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual RemovePackagePage.navigate(NormalMode, expectedAnswers).url

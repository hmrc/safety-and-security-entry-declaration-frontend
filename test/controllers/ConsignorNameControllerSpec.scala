...
class ConsignorNameControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val consignorNameRoute = routes.ConsignorNameController.onPageLoad(NormalMode, lrn).url
  ...
  "ConsignorName Controller" - {
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result          = route(application, request).value
        val expectedAnswers = emptyUserAnswers.set(ConsignorNamePage, "answer").success.value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual ConsignorNamePage.navigate(NormalMode, expectedAnswers).url

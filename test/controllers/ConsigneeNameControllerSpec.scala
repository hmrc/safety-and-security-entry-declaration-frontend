...
class ConsigneeNameControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val consigneeNameRoute = routes.ConsigneeNameController.onPageLoad(NormalMode, lrn, index).url
  ...
  "ConsigneeName Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(ConsigneeNamePage(index), "answer").success.value
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result          = route(application, request).value
        val expectedAnswers = emptyUserAnswers.set(ConsigneeNamePage(index), "answer").success.value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual ConsigneeNamePage(index).navigate(NormalMode, expectedAnswers).url

...
class NotifiedPartyIdentityControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val notifiedPartyIdentityRoute = routes.NotifiedPartyIdentityController.onPageLoad(NormalMode, lrn, index).url
  ...
  "NotifiedPartyIdentity Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(NotifiedPartyIdentityPage(index), NotifiedPartyIdentity.values.head).success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(NotifiedPartyIdentity.values.head), NormalMode, lrn, index)(request, messages(application)).toString
      }
    }
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result          = route(application, request).value
        val expectedAnswers = emptyUserAnswers.set(NotifiedPartyIdentityPage(index), NotifiedPartyIdentity.values.head).success.value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual NotifiedPartyIdentityPage(index).navigate(NormalMode, expectedAnswers).url

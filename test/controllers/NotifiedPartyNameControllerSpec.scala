...
class NotifiedPartyNameControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val notifiedPartyNameRoute = routes.NotifiedPartyNameController.onPageLoad(NormalMode, lrn, index).url
  ...
  "NotifiedPartyName Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(NotifiedPartyNamePage(index), "answer").success.value
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result          = route(application, request).value
        val expectedAnswers = emptyUserAnswers.set(NotifiedPartyNamePage(index), "answer").success.value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual NotifiedPartyNamePage(index).navigate(NormalMode, expectedAnswers).url

...
class RemoveConsigneeControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val removeConsigneeRoute = routes.RemoveConsigneeController.onPageLoad(NormalMode, lrn).url
  ...
  "RemoveConsignee Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(RemoveConsigneePage, true).success.value
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result          = route(application, request).value
        val expectedAnswers = emptyUserAnswers.set(RemoveConsigneePage, true).success.value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual RemoveConsigneePage.navigate(NormalMode, expectedAnswers).url

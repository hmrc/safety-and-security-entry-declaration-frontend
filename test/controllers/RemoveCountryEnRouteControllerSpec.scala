...
import views.html.RemoveCountryEnRouteView
...
class RemoveCountryEnRouteControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val removeCountryEnRouteRoute = routes.RemoveCountryEnRouteController.onPageLoad(NormalMode, lrn).url
  ...
  "RemoveCountryEnRoute Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(RemoveCountryEnRoutePage, true).success.value
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result          = route(application, request).value
        val expectedAnswers = emptyUserAnswers.set(RemoveCountryEnRoutePage, true).success.value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual RemoveCountryEnRoutePage.navigate(NormalMode, expectedAnswers).url

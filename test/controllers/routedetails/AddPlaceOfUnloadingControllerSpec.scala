...
class AddPlaceOfUnloadingControllerSpec extends SpecBase with MockitoSugar {
  ...
  "AddPlaceOfUnloading Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(AddPlaceOfUnloadingPage, true).success.value
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual AddPlaceOfUnloadingPage.navigate(NormalMode, expectedAnswers).url

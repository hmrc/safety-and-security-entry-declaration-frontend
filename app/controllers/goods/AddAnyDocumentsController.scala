...
class AddAnyDocumentsController @Inject() (
  ...
  def onPageLoad(waypoints: Waypoints, lrn: LocalReferenceNumber, itemIndex: Index): Action[AnyContent] =
    (cc.authAndGetData(lrn) andThen cc.limitIndex(itemIndex, maxGoods)) { implicit request =>
      ...
      val preparedForm = request.userAnswers.get(AddAnyDocumentsPage(itemIndex)) match {
        case None => form
    ...
    (cc.authAndGetData(lrn) andThen cc.limitIndex(itemIndex, maxGoods)).async { implicit request =>
      ...
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(AddAnyDocumentsPage(itemIndex), value))
              _ <- cc.sessionRepository.set(updatedAnswers)
            } yield Redirect(AddAnyDocumentsPage(itemIndex).navigate(waypoints, updatedAnswers))
        )
    }
}

...
class DangerousGoodCodeController @Inject() (
  formProvider: DangerousGoodCodeFormProvider,
  cc: CommonControllerComponents,
  view: DangerousGoodCodeView
  ...
  def onPageLoad(waypoints: Waypoints, lrn: LocalReferenceNumber, itemIndex: Index): Action[AnyContent] =
    (cc.authAndGetData(lrn) andThen cc.limitIndex(itemIndex, maxGoods)) { implicit request =>
      ...
      val preparedForm = request.userAnswers.get(DangerousGoodCodePage(itemIndex)) match {
        case None => form
    ...
    (cc.authAndGetData(lrn) andThen cc.limitIndex(itemIndex, maxGoods)).async { implicit request =>
      ...
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(DangerousGoodCodePage(itemIndex), value))
              _ <- cc.sessionRepository.set(updatedAnswers)
            } yield Redirect(DangerousGoodCodePage(itemIndex).navigate(waypoints, updatedAnswers))
        )
    }
}

...
import pages.goods.GoodsDescriptionPage
...
class GoodsDescriptionController @Inject() (
  ...
  def onPageLoad(waypoints: Waypoints, lrn: LocalReferenceNumber, itemIndex: Index): Action[AnyContent] =
    (cc.authAndGetData(lrn) andThen cc.limitIndex(itemIndex, maxGoods)) { implicit request =>
      ...
      val preparedForm = request.userAnswers.get(GoodsDescriptionPage(itemIndex)) match {
        case None => form
    ...
    (cc.authAndGetData(lrn) andThen cc.limitIndex(itemIndex, maxGoods)).async { implicit request =>
      ...
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(GoodsDescriptionPage(itemIndex), value))
              _ <- cc.sessionRepository.set(updatedAnswers)
            } yield Redirect(GoodsDescriptionPage(itemIndex).navigate(waypoints, updatedAnswers))
        )
    }
}

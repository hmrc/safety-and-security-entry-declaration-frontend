...
class DangerousGoodController @Inject() (
  formProvider: DangerousGoodFormProvider,
  cc: CommonControllerComponents,
  view: DangerousGoodView
  ...
  def onPageLoad(waypoints: Waypoints, lrn: LocalReferenceNumber, itemIndex: Index): Action[AnyContent] =
    (cc.authAndGetData(lrn) andThen cc.limitIndex(itemIndex, maxGoods)) { implicit request =>
      ...
      val preparedForm = request.userAnswers.get(DangerousGoodPage(itemIndex)) match {
        case None => form
    ...
    (cc.authAndGetData(lrn) andThen cc.limitIndex(itemIndex, maxGoods)).async { implicit request =>
      ...
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(DangerousGoodPage(itemIndex), value))
              _ <- cc.sessionRepository.set(updatedAnswers)
            } yield Redirect(DangerousGoodPage(itemIndex).navigate(waypoints, updatedAnswers))
        )
    }
}

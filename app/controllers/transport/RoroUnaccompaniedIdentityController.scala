...
class RoroUnaccompaniedIdentityController @Inject() (
  override val messagesApi: MessagesApi,
  sessionRepository: SessionRepository,
  identify: IdentifierAction,
  getData: DataRetrievalActionProvider,
  requireData: DataRequiredAction,
  formProvider: RoroUnaccompaniedIdentityFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: RoroUnaccompaniedIdentityView
  ...
  def onPageLoad(waypoints: Waypoints, lrn: LocalReferenceNumber): Action[AnyContent] =
    (identify andThen getData(lrn) andThen requireData) { implicit request =>
      ...
      val preparedForm = request.userAnswers.get(RoroUnaccompaniedIdentityPage) match {
        case None => form
    ...
    (identify andThen getData(lrn) andThen requireData).async { implicit request =>
      ...
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(RoroUnaccompaniedIdentityPage, value))
              _ <- sessionRepository.set(updatedAnswers)
            } yield Redirect(RoroUnaccompaniedIdentityPage.navigate(waypoints, updatedAnswers))
        )
    }
}

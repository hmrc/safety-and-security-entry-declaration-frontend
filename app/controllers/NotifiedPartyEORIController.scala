...
class NotifiedPartyEORIController @Inject()(
  ...
  def onPageLoad(mode: Mode, lrn: LocalReferenceNumber): Action[AnyContent] = (identify andThen getData(lrn) andThen requireData) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(NotifiedPartyEORIPage) match {
        case None => form
  ...
  def onSubmit(mode: Mode, lrn: LocalReferenceNumber): Action[AnyContent] = (identify andThen getData(lrn) andThen requireData).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(NotifiedPartyEORIPage, value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(NotifiedPartyEORIPage.navigate(mode, updatedAnswers))
      )
  }
}
